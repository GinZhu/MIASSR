...
class BraTSRawDataset(MIBasicTrain, MIBasicValid):
    """
    ...
    def __init__(self, data_folder, modalities, training_patient_ids=None, validation_patient_ids=None, dim=2,
                 margin=20, toy_problem=True, multi_threads=8, norm=''):
        super(BraTSRawDataset, self).__init__()
        ...
        # ## if not specified, randomly choose training the training and validation datasets
            ...
            all_HGG_patient_folder = glob(join(self.raw_data_folder, self.HGG_folder, 'Brats17*'))
            all_LGG_patient_folder = glob(join(self.raw_data_folder, self.LGG_folder, 'Brats17*'))

            HGG_patient_ids = ['HGG_{}'.format(_.split('/')[-1]) for _ in all_HGG_patient_folder]
            LGG_patient_ids = ['LGG_{}'.format(_.split('/')[-1]) for _ in all_LGG_patient_folder]

            self.all_HGGs = HGG_patient_ids
            self.all_LGGs = LGG_patient_ids

            np.random.shuffle(HGG_patient_ids)
            np.random.shuffle(LGG_patient_ids)
            HGG_patient_ids = HGG_patient_ids[:35]
            LGG_patient_ids = LGG_patient_ids[:15]

            training_patient_ids = HGG_patient_ids[7:] + LGG_patient_ids[3:]
            validation_patient_ids = HGG_patient_ids[:7] + LGG_patient_ids[:3]
...
class BraTSSRDataset(BraTSRawDataset):
    ...
    def __init__(self, paras):
        ...
        data_folder = paras.data_folder
        toy_problem = paras.toy_problem
        medical_image_dim = paras.medical_image_dim_brats
        training_patient_ids = paras.training_patient_ids_brats
        validation_patient_ids = paras.validation_patient_ids_brats
        margin = paras.margin_brats
        modalities = paras.modalities_brats
        multi_threads = paras.multi_threads
        norm = paras.normal_inputs

        super(BraTSSRDataset, self).__init__(
            data_folder=data_folder, modalities=modalities,
...
class BraTSSRTest(BraTSSRDataset):

    def __init__(self, paras, patient_id, data_folder=None, dim=None, sr_factor=None):
        test_paras = copy.deepcopy(paras)

        test_paras.validation_patient_ids_brats = [patient_id]
        test_paras.training_patient_ids_brats = []

        if data_folder:
            test_paras.data_folder = data_folder
        if dim:
            test_paras.medical_image_dim_brats = dim
        if sr_factor:
            test_paras.sr_scale = sr_factor

        super(BraTSSRDataset, self).__init__(test_paras)

        self.patient_id = patient_id


class BraTSMetaSRDataset(BraTSRawDataset):
    """
    Behaviours:
    ...
    def __init__(self, paras):
        ...
        super(BraTSMetaSRDataset, self).__init__(
            data_folder=data_folder, modalities=modalities,
            training_patient_ids=training_patient_ids,
            validation_patient_ids=validation_patient_ids, dim=medical_image_dim,
            margin=margin, toy_problem=toy_problem, multi_threads=multi_threads,
...
class BraTSMetaSRTest(BraTSMetaSRDataset):
    ...
    def __init__(self, paras, patient_id, test_sr_factors=None, dim=None):
        ...
        super(BraTSMetaSRTest, self).__init__(test_paras)
