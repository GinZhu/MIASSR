...
class BasicTester(ABC):
    ...
    def evaluation(self, case_name, DS):
        ...
        if exists(eva_report_file):
            eva_report = torch.load(eva_report_file)['eva_report']
            ...
            inference_time_cost = self.time_cost(self.current_time('float') - inference_start_time)
            # save only part of images
            inference_result = {
                'rec_imgs': self.select_images_to_save(rec_imgs),
                'inference_cost': inference_time_cost,
            }
            if self.save_gts:
