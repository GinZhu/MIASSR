    ...
    def train(self):
        for ts_i in range(self.current_training_state_id, len(self.training_states)):
            ...
            for i, training_batch in enumerate(DL, self.current_epoch+1):
                ...
                # add sr scales for ScaleGAN
                hr_size = target_imgs.size(3)
                lr_size = input_imgs.size(3)
                sr_scales = torch.ones_like(input_imgs[:, 0, 0, 0]) * (hr_size / lr_size)
                sr_scales = sr_scales.view(-1, 1)

                loss, repo = self.loss(rec_imgs, target_imgs, sr_scales)
